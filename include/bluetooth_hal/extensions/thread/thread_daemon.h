...
class ThreadDaemon {
  public:
     ...
     */
    explicit ThreadDaemon(std::optional<::bluetooth_hal::hci::HalPacketCallback> hal_packet_cb)
        ...
        CHECK(hal_packet_cb_ != std::nullopt) << __func__ << ": hal_packet_cb == nullptr";
  ...
  private:
    ...
    ::bluetooth_hal::hci::HalPacket ConstructToHalPacket(const std::vector<uint8_t>& packet);

    // Extracts the original packet data from a Hal packet.
    std::vector<uint8_t> ExtractFromHalPacket(const ::bluetooth_hal::hci::HalPacket& packet);
