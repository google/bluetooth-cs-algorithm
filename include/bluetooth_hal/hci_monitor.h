...
class HciMonitor {
  public:
    HciMonitor(MonitorType type, uint16_t primary_code, PacketDestination direction)
        : type_(type), primary_code_(primary_code), direction_(direction) {}

    void MonitorOffset(int offset, uint8_t data) { monitor_offset_map_[offset] = data; }

    void MonitorOffsetExclusion(int offset, uint8_t data) { exclude_offset_map_[offset] = data; }

    bool operator==(const HciMonitor& other) const {
        return type_ == other.GetType() && primary_code_ == other.GetPrimaryCode() &&
    ...
    bool operator==(const ::bluetooth_hal::hci::HalPacket& packet) const {
        ...
        } else if (packet.GetType() == ::bluetooth_hal::hci::HciPacketType::kEvent &&
                   type_ == MonitorType::kEvent) {
            type_match = true;
            packet_primary_code = packet.GetEventCode();
        } else if (packet.GetType() == ::bluetooth_hal::hci::HciPacketType::kThreadData &&
  ...
  protected:
    ...
    const std::map<int, uint8_t>& GetMonitorOffsets() const { return monitor_offset_map_; }
    const std::map<int, uint8_t>& GetMonitorOffsetExclusion() const { return exclude_offset_map_; }
...
class HciEventMonitor : public HciMonitor {
  public:
    ...
    HciEventMonitor(uint8_t event_code, uint8_t sub_event_code, int sub_event_offset)
        : HciMonitor(MonitorType::kEvent, static_cast<uint16_t>(event_code),
                     PacketDestination::kHost) {
...
class HciBleMetaEventMonitor : public HciEventMonitor {
  public:
    explicit HciBleMetaEventMonitor(uint8_t ble_event_code)
        : HciEventMonitor(static_cast<uint8_t>(EventCode::kBleMeta), ble_event_code,
...
class HciCommandCompleteEventMonitor : public HciEventMonitor {
  public:
    HciCommandCompleteEventMonitor(uint16_t command_opcode)
        ...
        MonitorOffset(HciConstants::kHciCommandCompleteCommandOpcodeOffset + 1, byte2);
    }
};
...
class HciCommandStatusEventMonitor : public HciEventMonitor {
  public:
    HciCommandStatusEventMonitor(uint16_t command_opcode)
        ...
        MonitorOffset(HciConstants::kHciCommandStatusCommandOpcodeOffset + 1, byte2);
    }
};
...
class HciCommandMonitor : public HciMonitor {
  public:
    explicit HciCommandMonitor(uint16_t opcode)
        : HciMonitor(MonitorType::kCommand, opcode, PacketDestination::kController) {}
...
class HciThreadMonitor : public HciMonitor {
  public:
    HciThreadMonitor() : HciMonitor(MonitorType::kThread, 0, PacketDestination::kHost) {}

    HciThreadMonitor(int offset, uint8_t data) : HciThreadMonitor() { MonitorOffset(offset, data); }
};
