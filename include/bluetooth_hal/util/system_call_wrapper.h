...
class SystemCallWrapper {
  public:
    ...
    /**
     * @brief Send a signal and data to a thread.
     *
     * @param tgid The thread group ID (process ID).
     * @param tid The thread ID.
     * @param sig The signal to send.
     * @param info A pointer to a siginfo_t structure containing signal data.
     *
     * @return On success, 0 is returned. On error, -1 is returned, and errno is
     * set appropriately.
     */
    virtual int RtTgsigqueueinfo(pid_t tgid, pid_t tid, int sig, siginfo_t* info) = 0;

    /**
     * @brief Set the abort message for the process.
     *
     * @param message The message to set.
     */
    virtual void SetAbortMessage(const char* message) = 0;
     ...
     *
     */
    virtual int Select(int nfds, fd_set* readfds, fd_set* writefds, fd_set* errorfds,
     ...
     *
     */
    virtual ssize_t Send(int fd, const void* buffer, size_t length, int flags) = 0;
     ...
     *
     */
    virtual int Accept(int fd, struct sockaddr* address, socklen_t* address_len) = 0;
     ...
     * @param whence Where to start the offset (SEEK_SET, SEEK_CUR, SEEK_END).
     *
     * @return On success, the resulting offset location. On error, -1 is
     * returned, and errno is set appropriately.
     */
     ...
     *
     */
    virtual int Bind(int fd, const struct sockaddr* address, socklen_t address_len) = 0;
    ...
    /**
     * @brief Sets the scheduling policy and parameters of a thread.
     *
     * @param thread The target thread handle.
     * @param policy The scheduling policy to set (e.g., SCHED_FIFO, SCHED_RR).
     * @param param A pointer to a sched_param structure containing the scheduling
     * parameters.
     *
     * @return On success, 0 is returned. On error, a positive error number is
     * returned.
     *
     */
    virtual int PthreadSetschedparam(pthread_t thread, int policy,
