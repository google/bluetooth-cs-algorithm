...
class ProviderFactory {
  public:
    ...
    static void RegisterProviderFactory(FactoryFn factory) {
        internal_factory_.RegisterProviderFactory(SingleProviderKey::kInstance, std::move(factory));
    }
  ...
  private:
    static inline MultiKeyProviderFactory<SingleProviderKey, Interface> internal_factory_;
};
...
struct hash<::bluetooth_hal::util::SingleProviderKey> {
    std::size_t operator()(const ::bluetooth_hal::util::SingleProviderKey& key) const {
