...
inline constexpr std::string_view SetupCommandTypeToString(SetupCommandType type) {
    switch (type) {
...
class SetupCommandPacket : public FirmwarePacket {
  public:
    SetupCommandPacket(SetupCommandType command_type, std::vector<uint8_t> payload)
...
class DataPacket : public FirmwarePacket {
  public:
    DataPacket(DataType data_type, std::vector<uint8_t> payload)
        : FirmwarePacket(FirmwarePacketType::kData, std::move(payload)), data_type_(data_type) {}
...
class FirmwareConfigLoader : public ConfigLoader {
  public:
     ...
     */
    virtual std::optional<std::reference_wrapper<const SetupCommandPacket>> GetSetupCommandPacket(
            SetupCommandType command_type) const = 0;
