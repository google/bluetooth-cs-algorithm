...
class HalConfigLoader : public ConfigLoader {
  public:
     ...
     * priorities.
     */
    virtual const std::vector<::bluetooth_hal::transport::TransportType>& GetTransportTypePriority()
            const = 0;
     ...
     */
    virtual const std::vector<std::string>& GetHwStagesWithoutLppControlBtPowerPin() const = 0;
    ...
    /**
     * @brief Check whether utilizes realtime (RT) scheduling.
     *
     * @return true if realtime thread are anbled: false otherwise.
     */
    virtual bool IsUsingRealtimeThread() const = 0;

    /**
     * @brief Gets the system time offset in microsecond.
     *
     * @return The system time offset, or -1 if not configured.
     */
    virtual int64_t GetSystemTimeOffsetUs() const = 0;
