...
enum class BqrReportId : uint8_t {
    ...
    // BqrEventType::kOffload
    kOffloadTimestamp = 0xF0,
};
...
inline BqrEventType GetBqrEventTypeFromReportId(BqrReportId id) {
    switch (id) {
        ...
        case BqrReportId::kOffloadTimestamp:
            return BqrEventType::kEventOffload;
        default:
...
enum class BqrErrorCode : uint8_t {
    ...
    kFirmwareInvalidResourceState = 0x18,
...
inline std::string_view BqrErrorToStringView(BqrErrorCode error_code) {
    switch (error_code) {
        ...
        case BqrErrorCode::kFirmwareInvalidResourceState:
            return "Firmware Crash due to Invalid resource state";
