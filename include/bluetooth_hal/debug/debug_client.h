...
#define CLIENT_LOG(severity)                          \
    ([](auto&& logger) -> auto&& { return logger; })( \
            this->ClientLog(::android::base::severity, LOG_TAG))
#endif
...
class DebugClient {
  public:
     ...
     */
    virtual void OnGenerateCoredump(CoredumpErrorCode error_code, uint8_t sub_error_code);
  ...
  protected:
     ...
     */
    ClientLogStream ClientLog(::android::base::LogSeverity severity, const char* tag);
    ...
    virtual std::vector<Coredump> Dump() { return {}; }
    const std::deque<std::string>& GetClientLogs() const {
