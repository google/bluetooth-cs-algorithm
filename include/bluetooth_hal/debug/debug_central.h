...
#else
#define SCOPED_ANCHOR(type, log)                                                                 \
    ::bluetooth_hal::debug::DurationTracker duration_##__COUNTER__(::bluetooth_hal::debug::type, \
                                                                   log);
#endif
...
class DebugCentral {
  public:
     ...
     */
    virtual void AddLog(AnchorType type, ::android::base::LogSeverity severity,
                        std::string_view log) = 0;
     ...
     */
    virtual void HandleDebugInfoEvent(const ::bluetooth_hal::hci::HalPacket& packet) = 0;
  ...
  public:
    ...
    ~LogHelper() {
        ...
        if (!log_message.empty()) {
            ...
            (void)type_;
#else
            DebugCentral::Get().AddLog(type_, severity_, log_message);
#endif
