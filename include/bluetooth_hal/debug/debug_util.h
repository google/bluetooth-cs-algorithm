...
inline std::string GenerateHalLogStringFrame(const std::string& title, const std::string& log,
                                             bool format_log = true) {
    std::stringstream ss;
    ss << "╔══════════════════════════════════════════════════════════\n";
...
inline bool IsBinFilePatternMatch(const std::string& filename, const std::string& base_prefix) {
    ...
    std::string timestamp_str =
            remaining_part.substr(0, remaining_part.length() - std::string(".bin").length());
    return std::regex_match(timestamp_str, kTimestampPattern);
}

inline void DeleteOldestBinFiles(const std::string& directory, const std::string& base_file_prefix,
    ...
    std::sort(filtered_files.begin(), filtered_files.end(), [](const auto& a, const auto& b) {
...
inline void FlushCoredumpToFd(int fd) {
    ...
    while ((entry = readdir(dir.get())) != nullptr) {
        ...
        if (stat(full_path.c_str(), &file_stat) == -1 || !S_ISREG(file_stat.st_mode)) {
            continue;
        }
        ...
        if (!input_file.is_open()) {
            current_file_content_ss << "ERROR: Failed to open file: " << full_path << std::endl;
        ...
        std::string formatted_log = GenerateHalLogStringFrame("LogFile: " + file_name,
                                                              current_file_content_ss.str(), false);
        combined_output_ss << formatted_log;
    }

    std::string final_output = combined_output_ss.str();
    if (!final_output.empty()) {
        ssize_t bytes_written = write(fd, final_output.c_str(), final_output.length());
        if (bytes_written == -1) {
            LOG(ERROR) << __func__ << ": Failed to write to file descriptor " << fd
                       << ". Error: " << strerror(errno);
        } else if (static_cast<size_t>(bytes_written) != final_output.length()) {
            LOG(WARNING) << __func__ << ": Incomplete write to file descriptor " << fd << ". Wrote "
                         << bytes_written << " of " << final_output.length() << " bytes.";
        }
    } else {
