...
class MockSystemCallWrapper : public SystemCallWrapper {
  public:
    ...
    MOCK_METHOD(int, RtTgsigqueueinfo, (pid_t tgid, pid_t tid, int sig, siginfo_t* info),
                (override));

    MOCK_METHOD(void, SetAbortMessage, (const char* message), (override));
    ...
    MOCK_METHOD(ssize_t, Send, (int fd, const void* buffer, size_t length, int flags), (override));

    MOCK_METHOD(ssize_t, Recv, (int fd, void* buffer, size_t length, int flags), (override));

    MOCK_METHOD(ssize_t, Write, (int fd, const void* buffer, size_t count), (override));

    MOCK_METHOD(ssize_t, Writev, (int fd, const struct iovec* iov, int iovcnt), (override));

    MOCK_METHOD(ssize_t, Read, (int fd, void* buffer, size_t count), (override));

    MOCK_METHOD(int, Accept, (int fd, struct sockaddr* address, socklen_t* address_len),
                (override));
    ...
    MOCK_METHOD(int, InotifyAddWatch, (int fd, const char* pathname, uint32_t mask), (override));

    MOCK_METHOD(int, Socket, (int domain, int type, int protocol), (override));

    MOCK_METHOD(int, Bind, (int fd, const struct sockaddr* address, socklen_t address_len),
                (override));
    ...
    MOCK_METHOD(int, PthreadSetschedparam,
                (pthread_t thread, int policy, const struct sched_param* param), (override));
